...
**Check for updates**: [Releases Page](https://github.com/ShivamB25/better-opencode-openai-codex-auth/releases)
...
```json
{
  "plugin": ["file:///absolute/path/to/better-opencode-openai-codex-auth/dist"]
}
```
...
**Build the plugin:**
```bash
cd better-opencode-openai-codex-auth
bun install
bun run build
```
